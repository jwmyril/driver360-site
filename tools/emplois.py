...
EMPLOYEURS = [
    ...
    dict(
        ...
        quoi={
            "en": "Amazon's official page for <strong>DSP delivery driver</strong> jobs, searchable by location. Each DSP is an <strong>independent company</strong> — you apply to it, and it is the one that hires you. Amazon's page says <strong>no CDL</strong> is needed and that the DSP provides the van, gas and insurance. DSP postings commonly ask for <strong>21 or over</strong> and a driving record and background check that meet the DSP's standards — check each posting.",
            "es": 'La página oficial de Amazon para empleos de <strong>conductor de reparto DSP</strong>, con búsqueda por ubicación. Cada DSP es una <strong>empresa independiente</strong> — postulas con ella, y es ella quien te contrata. La página de Amazon dice que <strong>no hace falta CDL</strong> y que el DSP pone la furgoneta, la gasolina y el seguro. Las ofertas de los DSP suelen pedir <strong>21 años o más</strong> y un historial de conducción y una verificación de antecedentes que cumplan los requisitos del DSP — lee cada oferta.',
            "ht": 'Paj ofisyèl Amazon pou travay <strong>chofè livrezon DSP</strong>, ou ka chèche selon kote w ye. Chak DSP se yon <strong>konpayi endepandan</strong> — se nan li ou aplike, epi se li ki anbochte w. Paj Amazon an di ou <strong>pa bezwen CDL</strong> epi se DSP a ki bay kamyonèt la, gaz la ak asirans lan. Òf DSP yo souvan mande <strong>21 an oswa plis</strong> ak yon dosye kondwi ak yon verifikasyon dosye ki koresponn ak sa DSP a mande — li chak òf.',
            "fr": "La page officielle d'Amazon pour les postes de <strong>chauffeur livreur DSP</strong>, avec recherche par lieu. Chaque DSP est une <strong>entreprise indépendante</strong> — c'est auprès d'elle que vous postulez, et c'est elle qui vous embauche. La page d'Amazon dit qu'<strong>aucun CDL</strong> n'est exigé et que le DSP fournit la camionnette, l'essence et l'assurance. Les offres des DSP demandent souvent <strong>21 ans ou plus</strong> et un dossier de conduite et une vérification d'antécédents conformes aux exigences du DSP — lisez chaque offre.",
        },
    ),
    dict(
